...
完成所有业务数据表的建立，用代码生成器生成 CRUD 骨架，后台菜单初始化完成。

---

## 1. 会员表扩展字段

在 ruoyi-vue-pro 的 `member_user` 表上添加业务字段。

### 1.1 Flyway Migration 脚本

- [ ] 创建文件：`maritime-biz/src/main/resources/db/migration/V1.0.0__member_extend.sql`

```sql
-- 扩展 member_user 表
-- 注：member_user 已有 status 字段（CommonStatusEnum: 1=ENABLE / 0=DISABLE）用于账号封禁，此处不重复添加
...
- [ ] 确认 Flyway 在启动时自动执行（`spring.flyway.enabled=true`）

---

## 2. 课程模块建表

- [ ] 创建文件：`V1.1.0__course_tables.sql`
...
-- 班期表
CREATE TABLE maritime_course_session (
  ...
  enrollment_deadline DATE     DEFAULT NULL COMMENT '报名截止日期（为空则使用费用配置的天数规则）',
...
-- 班期费用配置表（与班期 1:1）
-- ⚠️ 所有金额字段单位：元（DECIMAL），调用 ruoyi PayOrderApi / PayTransferApi 时需 ×100 转换为分（Integer）
CREATE TABLE maritime_session_fee_config (
  ...
  is_referral_commission_enabled TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否开启推荐佣金（0关闭 1开启）',
  is_deposit_refundable          TINYINT(1) NOT NULL DEFAULT 1 COMMENT '定金是否可退（0不可退 1可退）',
  balance_due_days_before_start  INT        NOT NULL DEFAULT 7  COMMENT '尾款截止：开班前N天（报名时据此计算 enrollment.balance_due_date）',
...
- [ ] 创建文件：`V1.2.0__enrollment_tables.sql`

```sql
-- 报名记录表
-- ⚠️ 所有金额字段单位：元（DECIMAL），调用 ruoyi PayOrderApi 时需 ×100 转换为分（Integer）
CREATE TABLE maritime_enrollment (
  id                      BIGINT        NOT NULL AUTO_INCREMENT,
  ...
  balance_due_date        DATE          DEFAULT NULL COMMENT '尾款截止缴纳日期（由后台配置或开班前N天自动算）',
  enrollment_status       VARCHAR(30)   NOT NULL DEFAULT 'PENDING_DEPOSIT'
                          COMMENT '报名状态(PENDING_DEPOSIT待支付定金←过渡态最长30分钟/DEPOSITED已付定金/PENDING_BALANCE待付尾款/IN_PROGRESS上课中/COMPLETED已完成/CANCELLED已取消/REFUNDING退款中)',
  ...
  -- 防止同一学员重复报名同班期（身份证+班期唯一）
  UNIQUE INDEX uk_enrollment_no (enrollment_no),
  UNIQUE INDEX uk_id_card_session (id_card, session_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='报名记录表';

-- 报名订单表
-- ⚠️ amount 单位：元（DECIMAL），调用 ruoyi PayOrderCreateReqDTO 时需 ×100 转换为分（Integer）
CREATE TABLE maritime_enrollment_order (
  id              BIGINT         NOT NULL AUTO_INCREMENT,
...
-- 退费申请表
-- ⚠️ refund_amount 单位：元（DECIMAL），调用 ruoyi PayRefundCreateReqDTO 时需 ×100 转换为分（Integer）
CREATE TABLE maritime_refund_apply (
  id              BIGINT      NOT NULL AUTO_INCREMENT,
  enrollment_id   BIGINT      NOT NULL COMMENT '报名ID',
  ...
  pay_refund_id   BIGINT      DEFAULT NULL COMMENT 'ruoyi pay_refund.id（退款发起后填写；实际微信退款单号在 pay_refund.channel_refund_no）',
...
- [ ] 创建文件：`V1.2.5__announcement_table.sql`

```sql
-- 最新动态/公告表（首页动态流 + 后台公告管理）
CREATE TABLE maritime_announcement (
  id           BIGINT       NOT NULL AUTO_INCREMENT,
...
- [ ] 创建文件：`V1.2.6__message_table.sql`

```sql
-- 用户站内消息表（小程序通知中心）
-- 系统自动写入（支付回调、拼团结果、佣金状态变更等），运营也可手动群发
CREATE TABLE maritime_user_message (
  id              BIGINT       NOT NULL AUTO_INCREMENT,
...
- [ ] 创建文件：`V1.3.0__groupon_tables.sql`

```sql
-- 拼团记录表（一次拼团发起）
CREATE TABLE maritime_groupon_record (
  id              BIGINT      NOT NULL AUTO_INCREMENT,
  session_id      BIGINT      NOT NULL COMMENT '班期ID',
...
-- 拼团成员表
CREATE TABLE maritime_groupon_member (
  id              BIGINT      NOT NULL AUTO_INCREMENT,
  ...
  tenant_id       BIGINT      NOT NULL DEFAULT 0,
  PRIMARY KEY (id),
  UNIQUE INDEX uk_groupon_enrollment (groupon_record_id, enrollment_id),
...
- [ ] 创建文件：`V1.4.0__referral_tables.sql`

```sql
-- 推荐关系表
CREATE TABLE maritime_referral_relation (
  id                    BIGINT   NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (id),
  -- 同一报名只能有一个推荐关系
  UNIQUE INDEX uk_referred_enrollment (referred_enrollment_id),
  INDEX idx_referrer_member (referrer_member_id),
  INDEX idx_session_id (session_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='推荐关系表';

-- 佣金记录表
-- ⚠️ commission_amount 单位：元（DECIMAL），调用 ruoyi PayTransferCreateReqDTO 时需 ×100 转换为分（Integer）
--    发放时：userAccount = 推荐人 member_user.open_id，channelExtras 需携带微信零钱发放 sceneId
--
-- 状态流转（与分佣流程.md 对齐）：
--   全款支付 → WAITING_FOR_CLASS（待开课）
--   开课后第7天，定时任务触发 → 检查退费
--     有退费 → FROZEN（冻结，人工处理）
--     无退费 → PENDING_REVIEW（进审核）
--   审核通过 → PENDING_PAYOUT
--   T+1 企业付款 → PAYING → PAID
--   失败 → FAILED → 人工补发 → MANUALLY_PAID
CREATE TABLE maritime_commission_record (
  id                    BIGINT         NOT NULL AUTO_INCREMENT,
  ...
  commission_status     VARCHAR(30)    NOT NULL DEFAULT 'WAITING_FOR_CLASS'
    COMMENT '状态(WAITING_FOR_CLASS待开课/FROZEN已冻结/PENDING_REVIEW待审核/REVIEWING审核中/PENDING_PAYOUT待发放/PAYING发放中/PAID已发放/REJECTED已拒绝/FAILED发放失败/MANUALLY_PAID人工处理)',
  expected_settle_date  DATE           NOT NULL COMMENT '预计结算日期（= 开课日期 + 7天，由定时任务按此日期触发审核）',
  is_risk_flagged       TINYINT(1)     NOT NULL DEFAULT 0 COMMENT '是否风控标记（0=普通待审/1=风险待审，对应后台审核队列的两个分组）',
  risk_check_result     JSON           DEFAULT NULL COMMENT '风控检查详情（JSON，含触发的具体规则说明）',
  ...
  pay_transfer_id       BIGINT         DEFAULT NULL COMMENT 'ruoyi pay_transfer.id（调用 PayTransferApi 后填写；userAccount=推荐人 open_id）',
...
-- 推荐人佣金账户（汇总）
CREATE TABLE maritime_commission_account (
  id              BIGINT        NOT NULL AUTO_INCREMENT,
  member_id       BIGINT        NOT NULL COMMENT '推荐人 member_id（唯一）',
  total_amount          DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '累计佣金总额',
  paid_amount           DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '已发放金额',
  pending_amount        DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '待发放金额',
  frozen_amount         DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '冻结中金额（审核/发放中）',
  rejected_amount       DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '被拒绝的佣金总额',
...
-- 风控黑名单
CREATE TABLE maritime_risk_blacklist (
  id          BIGINT      NOT NULL AUTO_INCREMENT,
  phone       VARCHAR(20) NOT NULL COMMENT '手机号',
...
- [ ] 打开管理后台 → 系统管理 → 代码生成 → 导入表
- [ ] 选择以下表，配置生成参数：
  - 模块名：`maritime`
  - 包路径：`cn.iocoder.yudao.module.maritime`
  - 业务名（每张表不同）

| 表名 | 业务名 | 类名前缀 |
|------|--------|---------|
| maritime_course | course | Course |
| maritime_course_session | session | CourseSession |
| maritime_session_fee_config | feeConfig | SessionFeeConfig |
| maritime_enrollment | enrollment | Enrollment |
| maritime_enrollment_order | enrollmentOrder | EnrollmentOrder |
| maritime_refund_apply | refundApply | RefundApply |
| maritime_announcement | announcement | Announcement |
| maritime_user_message | userMessage | UserMessage |
| maritime_groupon_record | grouponRecord | GrouponRecord |
| maritime_groupon_member | grouponMember | GrouponMember |
| maritime_referral_relation | referralRelation | ReferralRelation |
| maritime_commission_record | commissionRecord | CommissionRecord |
| maritime_commission_account | commissionAccount | CommissionAccount |
| maritime_risk_blacklist | blacklist | RiskBlacklist |

- [ ] 生成并下载代码
- [ ] 将生成的代码放入 `maritime-biz` 模块对应目录
- [ ] 编译验证，确认无报错
...
```
海员培训（目录）
  ├─ 课程管理（菜单） → /maritime/course
  ├─ 班期管理（菜单） → /maritime/session
  ├─ 报名管理（菜单） → /maritime/enrollment
  ├─ 订单管理（菜单） → /maritime/order
  ├─ 拼团管理（菜单） → /maritime/groupon
  ├─ 推荐管理（菜单） → /maritime/referral
  ├─ 佣金管理（菜单） → /maritime/commission
  ├─ 黑名单管理（菜单） → /maritime/blacklist
  └─ 财务统计（菜单） → /maritime/finance
```
...
- [ ] 所有 SQL Migration 脚本执行成功，表结构正确
- [ ] 所有表的 Mapper / Service / Controller 骨架代码已生成
- [ ] 后台管理菜单可见（即使页面还是空白）
- [ ] 后端启动无报错
- [ ] Swagger 中可以看到 maritime 模块的接口列表
